          ...
          <div>
            ...
            <div class="opciones-pestañas">
              <b-col class="pestaña" :class="this.pestañaActiva === 'Histórico' ? 'active' : 'inactive'
                " @click="cambiarPestaña('Histórico')"><b-icon icon="clock"></b-icon> Horas</b-col>
  ...
  methods: {
    ...
    async guardarFirma() {
      try {
        ...
        formData.append("idVisita", this.idVisitaCerrar);

        try {
          // Envía los datos al servidor
          await this.$axios.post('/api/visitas/close', formData, {
            headers: {
          ...
          // Procesa la respuesta y maneja el éxito o error aquí
          this.$refs.modalFirma.hide();
          this.$router.go(-1);
        } catch (error) {
          console.error('Error al enviar la firma y cerrar la visita', error);
        }
      } catch (error) {
        console.error(error); // Maneja el error en caso de que la firma esté vacía o surja otro problema
      }
    },
    ...
    onArticuloChange(newValue) {
      ...
      this.historialAEditar["VisitasLineas::linartcodref"] = articuloSeleccionado.value;
      // Actualiza form.DescripciónArt con el texto del artículo seleccionado
      this.historialAEditar["VisitasLineas::DescripciónArt"] = articuloSeleccionado.text;
    },
    ...
    handleEdit() {
      ...
      let data = {
        linartcodref: this.historialAEditar['VisitasLineas::linartcodref'],
